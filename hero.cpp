...
void Hero::guide(const std::vector<Villager*>& villagers)
{
    ...
    int choice = Game::getIntNumber("What Villager you want to guide?") ;

    while (choice > static_cast<int>(nearVillager.size() + inVillagers.size()) )
    {
        choice = Game::getIntNumber("Enter a correct choice!! :");
    }
    ...
    if (choice <= static_cast<int>(nearVillager.size() ) )
    {
        ...
        int choose = Game::getIntNumber("Enter location that villager should go :");
        while (choose > static_cast<int>(nearlocations.size()))
        {
            choose = Game::getIntNumber("Enter a correct choice!! :");
        }
...
void Hero::advanceTask(Monster* monster) {
    if (monster->getName() == MonsterName::DRACULA)
    {
        try
        {
            ...
            for (size_t i = 0; i < items.size(); i++)
            {
                int entekhab = Game::getIntNumber("Enter the Item you want to use (100 for end):");
                if (entekhab == 100)
                {
                    break;
                }
        ...
        try
        {
            ...
            int entekhab = Game::getIntNumber("Enter the Item you want to place :");
...
void Hero::pickUpItem(const std::array<Item* , 70>& AllItems)
{ 
    if (name == HeroName::ARCHAEOLOGIST)
    {
        ...
        while (true)
        {
            itemWant = Game::getIntNumber("Enter what Items you want to pick (100 for end):");
...
bool Hero::receiveAttack() noexcept{
    ...
    if (answer == "Y" || answer == "y")
    {
        ...
        int perk = Game::getIntNumber("Choose the perk you want to use :");
